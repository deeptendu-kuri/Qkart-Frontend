...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  const nextCart = cartData.map((item)=>{
    const productData = productsData.find((product)=>
...
export const getTotalItems = (items = []) => {
  if (!items.length) return 0;
  const total=items.map((item)=>item.qty).reduce((total,n)=>total+n,0)
  return total;
};
...
const Cart = ({
  ...
  <Box key={item.productId}>
    {item.qty > 0 && (
      ...
      {isReadOnly && <Box className="checkOut-cart" padding="1rem">
        <h2>Order Details</h2>
        ...
        <Box className="cart-row">
          <h3>Total</h3>
          <p style={{ fontWeight: 700 }}>${getTotalCartValue(items)}</p>
        </Box>
      </Box>}
    </>
  );
