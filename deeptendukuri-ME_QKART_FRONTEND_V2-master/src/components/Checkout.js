...
const Checkout = () => {
  ...
  useEffect(()=>{
    if(token){
      ...
      enqueueSnackbar("You must be loggedin to access checkout page",{
        variant:"info"
      })
      history.push("/")
    }
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(localStorage.getItem("balance")<getTotalCartValue(items)){
      enqueueSnackbar("You dodnot have enough balance in your wallet for this purchase.",{
        variant:"warning"
      })
      return false
    };
      if(!addresses.all.length){
        enqueueSnackbar("Please add a new address before proceedding.",{
          variant:"warning"
        })
        return false;
    }
    ...
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart isReadOnly={true} hasCheckOutButton products={products} items={items} />
        </Grid>
      </Grid>
      <Footer />
    </>
